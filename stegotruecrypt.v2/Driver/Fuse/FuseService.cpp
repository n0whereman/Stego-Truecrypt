	...
	void FuseService::CloseMountedVolume ()
	{
		if (MountedVolume)
		{
			...
			if (MountedVolume->GetFile().use_count() > 1) {
				/*if (typeid (*MountedVolume->GetFile()) == typeid (FileStego))
					(dynamic_pointer_cast<FileStego>(MountedVolume->GetFile()))->SetFuse(false,true);*/
				SystemLog::WriteError("Gonna close FILE from FuseService::CloseMountedVolume" + string(MountedVolume->GetPath()));				
