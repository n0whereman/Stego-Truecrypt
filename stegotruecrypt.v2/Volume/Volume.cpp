...
//#include "wx/filefn.h" //?????????????????????????????? vhyba wx/setup avsak ten ma byt vysledkom kompilkovania
//momo pridana filefn.h
namespace TrueCrypt
{
	Volume::Volume ()
	...
	void Volume::Open (const VolumePath &volumePath, bool preserveTimestamps, shared_ptr <VolumePassword> password, shared_ptr <KeyfileList> keyfiles, VolumeProtection::Enum protection, shared_ptr <VolumePassword> protectionPassword, shared_ptr <KeyfileList> protectionKeyfiles, bool sharedAccessAllowed, VolumeType::Enum volumeType, bool useBackupHeaders, bool partitionInSystemEncryptionScope)
	{
		...
		wstring path = wstring(volumePath);
		...
		// UNIX!!! TODOOOOOOOO, checking DIR
		struct stat statData;
		throw_sys_sub_if (stat (StringConverter::ToSingle (path).c_str(), &statData) != 0, wstring(volumePath));
		if (S_ISDIR (statData.st_mode)) {
			file = make_shared<FileStego>(); 
			shared_ptr<CarrierFilesManager> manager = shared_ptr<CarrierFilesManager>(new CarrierFilesManager());
			manager->setPassword("q"); //HARDCODEEEED! :(
			manager->loadDirectory(string(volumePath).c_str());
			shared_ptr<Encoder> e = EncoderFactory::getEncoderByName("lsb"); //HARDCODEEEED! :(
			EncoderFactory::setEncoderArgByName(e,"blockSize", "1");         //HARDCODEEEED! :(
			manager->setEncoder(e);                                         //EncoderFactory::getEncoderByName("lsb")
			manager->applyEncoder();
			(dynamic_pointer_cast<FileStego>(file))->SetManager(manager);
			//(dynamic_cast<FileStego*> (file.get()))->SetFuse(true,false);
			(dynamic_pointer_cast<FileStego>(file))->SetFuse(true,false);
		} //else  make_shared_auto (File, file);
	...
	void Volume::Open (shared_ptr <File> volumeFile, shared_ptr <VolumePassword> password, shared_ptr <KeyfileList> keyfiles, VolumeProtection::Enum protection, shared_ptr <VolumePassword> protectionPassword, shared_ptr <KeyfileList> protectionKeyfiles, VolumeType::Enum volumeType, bool useBackupHeaders, bool partitionInSystemEncryptionScope)
	{
		...
		{
			Close(); ///TODOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO may be error
			throw;
		}
	}
