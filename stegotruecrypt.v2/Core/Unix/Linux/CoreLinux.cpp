...
namespace TrueCrypt
{
	...
	DevicePath CoreLinux::AttachFileToLoopDevice (const FilePath &filePath, bool readOnly) const
	{
		...
		SystemLog::WriteError("AttachFileToLoopDevice");
	...
	void CoreLinux::MountVolumeNative (shared_ptr <Volume> volume, MountOptions &options, const DirectoryPath &auxMountPoint) const
	{
		...
		try
		{
			...
			SystemLog::WriteError("MountNativePath volumePath: "+string(volumePath));
			...
			File nativeDev;
			SystemLog::WriteError("MountVolumeNative nativeDevPath="+nativeDevPath);
			...
			if (!options.NoFilesystem && options.MountPoint && !options.MountPoint->IsEmpty())
			{
				...
				SystemLog::WriteError("MountVolumeNative 08");
			}

			SystemLog::WriteError("MountVolumeNative 09");
		...
		catch (...)
		{
			try
			{
				SystemLog::WriteError("MountVolumeNative 10");
			...
			try
			{
				if (nativeDevCreated)
				{
					SystemLog::WriteError("MountVolumeNative 11");
			...
			try
			{
				SystemLog::WriteError("MountVolumeNative 12");
				if (loopDevAttached)
		...
		SystemLog::WriteError("MountVolumeNative 13 END");
	}
