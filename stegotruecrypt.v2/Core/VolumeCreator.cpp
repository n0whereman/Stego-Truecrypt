...
namespace TrueCrypt
{
	...
	void VolumeCreator::CreationThread ()
	{
		try
		{
            ...
            cerr << "endOffset je " << endOffset << endl;
			VolumeFile->SeekAt (DataStart);
			cerr << "Pred create filesystem " << endl;
			// Create filesystem
				...
				struct WriteSectorCallback : public FatFormatter::WriteSectorCallback
				{
					...
					void FlushOutputBuffer ()
					{
						if (OutputBufferWritePos > 0)
						{
							...
							cerr  << "Bol zapisany OutputBufferWritePos" << endl;
         ...
         ///tu som
			if (!Options->Quick)
			{
				...
				cerr << "outputBuffersize je " << outputBuffer.Size() << endl;
				if(Stego)
				endOffset = (HostSize - 131072); 

				while (!AbortRequested && WriteOffset < endOffset) //WriteOffset < endOffset
				{
					if (WriteOffset + dataFragmentLength > endOffset) {    //(WriteOffset + dataFragmentLength > endOffset)
						dataFragmentLength = endOffset - WriteOffset; //endOffset - WriteOffset
						cerr << "dataFragmentLength je " << dataFragmentLength << endl;
					}
					...
					cerr << "WriteOffset je " << WriteOffset << endl;
	...
	void VolumeCreator::CreateVolume (shared_ptr <VolumeCreationOptions> options)
	{
        ...
        
		{
...
#endif
			if(options->HostType == VolumeHostType::StegoFS) {
				Stego = true;
				VolumeFile.reset (new FileStego);
				(dynamic_pointer_cast<FileStego>(VolumeFile))->SetManager(options->Manager);
				(dynamic_pointer_cast<FileStego>(VolumeFile))->SetFuse(false,false);
				if(VolumeFile == nullptr)
				   throw SystemException (SRC_POS, L"StegoManager was not applied");
			...
			SystemLog::WriteError("CreateVolume: HostSize=" + std::to_string(HostSize));
		}
		...
		try
		{
			...
			// mY TODO: it is necessary set data size as multiple of sectorSize
			// mY TODO: see Volume/Volume.cpp: Volume::ReadSectors
			// mY TODO: if (length % SectorSize || byteOffset % SectorSize ) ---> throw ParameterIncorrect
			options->Size -= options->Size % options->SectorSize;
			cerr << "options->SectorSize je " << options->SectorSize << endl;
			...
			cerr << "GetHeaderSize je " << Layout->GetHeaderSize() << endl;
			...
			cerr << "headerOptions.SectorSize je "  << options->SectorSize << endl;
			...
			cerr << "headerOptions.VolumeDataStart je " << headerOptions.VolumeDataStart << endl;

			headerOptions.VolumeDataSize = Layout->GetMaxDataSize (options->Size);
			SystemLog::WriteError("CreateVolume: VolumeDataSize=" + std::to_string(headerOptions.VolumeDataSize) + ", VOL_HEAD_SIZE="+std::to_string(TC_TOTAL_VOLUME_HEADERS_SIZE));
            ...
            cerr << "Layout->GetHeaderOffset() je " << Layout->GetHeaderOffset() << endl;
			// Write new header
           ...
           // throw SystemException (SRC_POS, L"pred write");
			VolumeFile->Write (headerBuffer);
            //
            //throw SystemException (SRC_POS, L"po write zapisany header");
			if (options->Type == VolumeType::Normal)
			{
