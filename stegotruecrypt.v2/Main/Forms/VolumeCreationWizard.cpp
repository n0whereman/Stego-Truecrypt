...
namespace TrueCrypt {
    ...
    WizardPage *VolumeCreationWizard::GetPage(WizardStep step) {
        switch (step) {
            ...
            {
                manager = shared_ptr<CarrierFilesManager>(new CarrierFilesManager()); //vytvoreny carrierfilesmanager
                ClearHistory();
                StegoVolume = false;
                OuterVolume = false;
                LargeFilesSupport = false;
                ...
                page->AddChoice(VolumeHostType::StegoFS, LangString["IDC_STEGO_CONTAINER"], LangString["IDT_STEGO_CONTAINER"]); //momo
            ...
            case Step::VolumeLocation:
            {
                ...
                /*else if (SelectedVolumeType == VolumeType::Stego)
                    page->SetPageText(LangString[SelectedVolumeHostType == VolumeHostType::StegoFS ? "FILE_HELP_STEGO" : "DEVICE_HELP_HIDDEN_HOST_VOL"]);*/
                else
            ...
            case Step::EncryptionOptions: // momo
            {
            ...
            case Step::EncoderOptions: // momo
            { 
                
                EncoderOptionsWizardPage *page = new EncoderOptionsWizardPage(GetPageParent(),manager);                
                page->SetPageTitle (LangString["STEGO_TITLE"]);
                page->SetEncoder (SelectedEncoder);  
                page->SetMinVolumeSize(TC_MIN_HIDDEN_VOLUME_HOST_SIZE); 

              return page;
            }

            case Step::EncoderVolumeSize: // uplne zmenene momo
            {
                EncoderVolumeSizePage *page = new EncoderVolumeSizePage(GetPageParent());
                page->SetPageTitle(LangString["STEGO_TITLE"]);
                //page->SetEncoder (SelectedEncoder);   
        ...
        try {
            if (!CreationAborted) {
                ...
                switch (SelectedFilesystemType) {
                    case VolumeCreationOptions::FilesystemType::Ext2: fsFormatter = "mkfs.ext2";
                        break;
                    case VolumeCreationOptions::FilesystemType::Ext3: fsFormatter = "mkfs.ext3";
                        break;
                    case VolumeCreationOptions::FilesystemType::Ext4: fsFormatter = "mkfs.ext4";
                        break;
                    case VolumeCreationOptions::FilesystemType::MacOsExt: fsFormatter = "newfs_hfs";
                        break;
                    case VolumeCreationOptions::FilesystemType::UFS: fsFormatter = "newfs";
                        break;
                    default: break;
                }
    ...
    WizardFrame::WizardStep VolumeCreationWizard::ProcessPageChangeRequest(bool forward) {
        switch (GetCurrentStep()) {
            case Step::VolumeHostType:
            {
                ...
                if (SelectedVolumeHostType == VolumeHostType::StegoFS)
                    StegoVolume = true;
            ...
            case Step::VolumeLocation:
            {
                ...
                SelectedVolumePath = page->GetVolumePath(); // momo  TODO potrebujem to premenit na STRING!!  kvoli stegomanager
                
                VolumeSize = 0;
                manager->loadDirectory(((string) SelectedVolumePath).c_str()); //momo CarrierFilesManager was created"
            ...
            case Step::EncoderOptions:
            {
                    EncoderOptionsWizardPage *page = dynamic_cast<EncoderOptionsWizardPage *> (GetCurrentPage());
                    try {
                          SelectedEncoder = page->GetParameterizedEncoder();
                           manager->setEncoder(SelectedEncoder); //EncoderFactory::getEncoderByName("lsb")
                           manager->applyEncoder();
                           if (manager->getCapacity() == 0) {
                                 LOG_ERROR("Not enough stego space in carrier files");
                                return 1;
                          }
                          VolumeSize = manager->getCapacity(); // momo
            ...
            case Step::EncoderVolumeSize:
            {
                EncoderVolumeSizePage *page = dynamic_cast<EncoderVolumeSizePage *> (GetCurrentPage());
            ...
            case Step::VolumePassword:
            {
                ...
                std::string Pass = page->GetPasswordString();
                ...
                if (StegoVolume) {
                    ...
                    manager->setPassword(Pass.c_str());
    
                    return Step::EncoderOptions;
                }    
            ...
            case Step::CreationProgress:
            {
                ...
                if (forward) {
                    ...
                    try {
                        ...
                        if(StegoVolume) {
                           options->HostType = VolumeHostType::StegoFS;
                        }
                        ...
                        //throw SystemException (SRC_POS, L"pred creator.reset");
